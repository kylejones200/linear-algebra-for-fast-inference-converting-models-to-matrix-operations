...
def calculate_inference_metrics(x_true: np.ndarray, x_pred: np.ndarray) -> dict:
    ...
    return {'mse': np.mean((x_true - x_pred) ** 2), 'mae': np.mean(np.abs(x_true - x_pred)), 'correlation': np.corrcoef(x_true, x_pred)[0, 1]}
...
def plot_inference_results(x_true: np.ndarray, x_pred: np.ndarray, title: str, output_path: Path, plot: bool=False):
    ...
    ax.plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=1.2, label='Perfect Inference')
